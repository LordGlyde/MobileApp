...
public class quizActivity extends AppCompatActivity {
    ...
    public void generateBugtong(){
        int currentIndex = quiz.questionShown;
    ...
    public void checkUserAnswer(String choice){
        ...
        if(bugtong.answer[currentIndex].equalsIgnoreCase(choice)){
            ...
            quiz.SCORE -= 5;
            uicToastMessage("Wrong! Answer is " + bugtong.answer[currentIndex]);
        }
        textView_score.setText(quiz.SCORE+"");
    }

    public void setAnswerCorrect(){
        uicToastMessage("Correct!");
        quiz.SCORE += (5 * (int) remainingSeconds);
        generateBugtong();
    }


    public void saveUserData(){
        boolean insertData = databaseHelper.addData(currentUsername, quiz.SCORE);
        if(insertData) {
            timer.cancel();
            startActivity(new Intent(quizActivity.this, TallyActivity.class));
        }else{
